...
CONFIG_ARGS = [
    ...
    [('--unoparty-rpc-connect',), {'default': 'localhost', 'help': 'the hostname or IP of the Unoparty JSON-RPC server'}],
    [('--unoparty-rpc-port',), {'type': int, 'help': 'the port of the Unoparty JSON-RPC server'}],
    [('--unoparty-rpc-user',), {'default': 'rpc', 'help': 'the username for the Unoparty JSON-RPC server'}],
    [('--unoparty-rpc-password',), {'help': 'the password for the Unoparty JSON-RPC server'}],
    [('--unoparty-rpc-ssl',), {'default': False, 'action': 'store_true', 'help': 'use SSL to connect to the Unoparty server (default: false)'}],
    [('--unoparty-rpc-ssl-verify',), {'default': False, 'action': 'store_true', 'help': 'verify SSL certificate of the Unoparty server; disallow use of self‐signed certificates (default: false)'}],

    [('--wallet-name',), {'default': 'unobtaniumcore', 'help': 'the wallet name to connect to'}],
    ...
    [('--wallet-user',), {'default': 'unobtaniumrpc', 'help': 'the username used to communicate with wallet'}],
...
def main():
    ...
    parser = argparse.ArgumentParser(prog=APP_NAME, description='Unoparty CLI for unoparty-server', add_help=False)
    parser.add_argument('-h', '--help', dest='help', action='store_true', help='show this help message and exit')
    parser.add_argument('-V', '--version', action='version', version="{} v{}; {} v{}".format(APP_NAME, APP_VERSION, 'unoparty-lib', config.VERSION_STRING))
    ...
    parser_send.add_argument('--no-use-enhanced-send', action='store_false', dest="use_enhanced_send", default=True, help='If set to false, compose a non-enhanced send with a unobtanium dust output')
    ...
    parser_issuance.add_argument('--meltable', action='store_true', help='whether or not the asset is backed by another asset')
    ...
    parser_bet.add_argument('--wager', required=True, help='the quantity of XUP to wager')
    parser_bet.add_argument('--counterwager', required=True, help='the minimum quantity of XUP to be wagered by the user to bet against you, if he were to accept the whole thing')
    ...
    parser_dividend.add_argument('--quantity-per-unit', required=True, help='the quantity of XUP to be paid per whole unit held of ASSET')
    ...
    parser_burn = subparsers.add_parser('burn', help='destroy {} to earn XUP, during an initial period of time')
    ...
    parser_destroy = subparsers.add_parser('destroy', help='destroy a quantity of a Unoparty asset')
    ...
    parser_getrows = subparsers.add_parser('getrows', help='get rows from a Unoparty table')
    ...
    clientapi.initialize(testnet=args.testnet, testcoin=args.testcoin, regtest=args.regtest, customnet=args.customnet,
                        unoparty_rpc_connect=args.unoparty_rpc_connect, unoparty_rpc_port=args.unoparty_rpc_port,
                        unoparty_rpc_user=args.unoparty_rpc_user, unoparty_rpc_password=args.unoparty_rpc_password,
                        unoparty_rpc_ssl=args.unoparty_rpc_ssl, unoparty_rpc_ssl_verify=args.unoparty_rpc_ssl_verify,
